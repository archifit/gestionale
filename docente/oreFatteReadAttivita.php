	...
	while($row = mysqli_fetch_assoc($result)) {
		...
		if ($row['ore_previste_tipo_attivita_inserito_da_docente']) {
			if ($modificabile) {
			    ...
			    if ($row['ore_fatte_attivita_contestata'] == 1) {
			        $data .='
    				    <button onclick="oreFatteControllaAttivita('.$row['ore_fatte_attivita_id'].', \''.$row['ore_fatte_attivita_dettaglio'].'\','.$row['ore_fatte_attivita_ore'].')" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-pencil"></button>
				    ';
			    } else {
			        $data .='
    				<button onclick="oreFatteControllaAttivita('.$row['ore_fatte_attivita_id'].', \''.$row['ore_fatte_attivita_dettaglio'].'\','.$row['ore_fatte_attivita_ore'].')" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-pencil"></button>
				';
			    }
			}
		}
